...
const fov: GuidePageData = {
  ...
  title: "Mistfall Hunter FOV Settings and Camera Options Explained",
...
const bestSettings: GuidePageData = {
  ...
  title: "Mistfall Hunter Best Settings: Stable PC & Console Setup",
...
const controller: GuidePageData = {
  ...
  title: "Mistfall Hunter Controller Guide: Setup and Safe Fixes",
...
const sharedFixSections = (settingsStep: string, errorCapture: string): GuideSection[] => [
  {
    ...
    table: {
      headers: ["Order", "Action", "Success or next step"],
      rows: [
        ["1", "Check current official issue and update posts", "Install a named fix before changing the system"],
        ["2", "Restart, then verify or repair files through the platform", "Retest the same failure stage"],
        ["3", "Use supported system updates and close only nonessential overlays", "Restore anything that makes no difference"],
        ["4", settingsStep, "Keep the change only if the same test improves"],
  ...
  },
  {
    ...
    bullets: [
      errorCapture,
      "Platform, store, game version, and the exact stage where the symptom begins.",
      "The one change tested and whether the same scenario improved, failed, or changed symptoms.",
      "Relevant PC hardware and driver versions, or the exact console model and system version.",
      "A screenshot or short recording with account and personal information removed.",
    ],
  },
  {
    heading: "Stop conditions and unsafe boundaries",
    paragraphs: [
      "Stop local testing when the issue affects many unrelated players, matches a current official incident, persists after the supported baseline, or requires account-side investigation. Send the evidence above through in-game support or an official Bellring Games channel.",
      "Do not download replacement DLLs, registry bundles, or generic fixer tools. Do not disable anti-cheat or security, change BIOS or voltage settings, or use unknown launch flags as first-line troubleshooting.",
    ],
  },
];
...
const fatal = fixPage({
  ...
  title: "Mistfall Hunter Fatal Error Fix: Safe Troubleshooting",
...
const stutter = fixPage({
  ...
  title: "Mistfall Hunter Stuttering Fix: Safe Frame-Pacing Steps",
...
const crashing = fixPage({
  ...
  title: "Mistfall Hunter Crashing Fixes for PC, PS5 and Xbox",
...
const connection = fixPage({
  ...
  title: "Mistfall Hunter Connection Error Fix and Network Checks",
